...
class VideoPlayer(QWidget):
    def __init__(self, args):
        ...
        self.video_lang_input.setFixedHeight(90)
        ...
        self.clip_lang_input.setFixedHeight(120)
        ...
        if self.mode == 'SAM首次标注':
            ...
            tips_items = ['W: 标志开始帧','F: 添加段语言标注','S: 标记结束帧','删除: 删除标记帧','A: 上一帧','回车: 添加视频标注','D: 下一帧']
    ...
    def file_load(self, args):
        ...
        file_select_dialog.setFixedSize(700, 250)
        ...
        self.file_load_path_label.setFixedSize(300,70)
        # 自动换行
    ...
    def mode_choose(self):
        ...
        mode_dialog.setFixedSize(500, 200)
    ...
    def load_video_callback(self, res):
        ...
        if video is not None:
            ...
            if self.video_cache.get(self.video_list[self.cur_video_idx]) is None and \
                self.video_cache.get_locate(self.video_list[self.cur_video_idx-1]) < int(CACHE_NUMBER/2) and \
                    self.video_cache.__len__() < CACHE_NUMBER:
    ...
    def load_video_quiet_callback(self, res):
        ...
        if video is not None:
            ...
            for video_id in range(self.cur_video_idx, len(self.video_list)):
                if self.video_cache.get_locate(self.video_list[self.cur_video_idx-1]) == int(CACHE_NUMBER/2):
                    self.video_cache.pop()
                if self.video_cache.__len__() >= CACHE_NUMBER:
                    break
...
if __name__ == "__main__":
    ...
    player.resize(1600, 700)  # Adjusted size to accommodate the toolbar
    player.show()
    sys.exit(app.exec_())